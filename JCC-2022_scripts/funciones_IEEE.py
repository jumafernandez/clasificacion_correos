...
def cargar_dataset(URL_data, file, descarga, atr_X, atr_y, CANTIDAD_INSTANCIAS=None):
    ...
    # Me fijo si debo tomar una columna determinada (consulta) o todas
    if atr_X:
        X = df[atr_X]
    else:
        all_columns = list(df.columns)
        all_columns.remove(atr_y)
        X = df[all_columns]
...
def get_BoW(X, representacion, MAX_TKS=None, X_test=None):
    ...
    X_vec_df = pd.DataFrame.sparse.from_spmatrix(X_vec, columns=vectorizer.get_feature_names())
